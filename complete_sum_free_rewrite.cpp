...
void write_sets_to_file(set_bound_t n) {
	...
	if (n < 10) {
		outfile.open("output_files/complete_sum_free_sets_0" + std::to_string(n) + ".dot", std::ofstream::out | std::ofstream::trunc);
	}
	else {
		outfile.open("output_files/complete_sum_free_sets_" + std::to_string(n) + ".dot", std::ofstream::out | std::ofstream::trunc);
	}
...
uint32_t print_all_complete_sum_free_sets(set_bound_t n) {
